...
CHANNEL_USERNAME = "@MrTripleR_YT0"

# ইউজারের চ্যানেল সাবস্ক্রিপশন চেক করার ফাংশন
...
# ফোর্স সাবস্ক্রাইব পেজ (প্রাইভেট চ্যাটের জন্য)
...
# ১. শুধুমাত্র /start স্ল্যাশ কমান্ড হ্যান্ডলার
async def start_command(update: Update, context):
    if update.effective_chat.type != "private":
        await update.message.reply_text("👋 Hello! I am **TYNEX VIDEO DOWNLOADER**. Send me any video link in private chat to download! 🚀", parse_mode="Markdown")
        return
...
# ২. মূল প্রিমিয়াম মেনু (বাটন ভিত্তিক)
...
# ৪. গ্রুপে বট যুক্ত হলে নোটিফিকেশন
async def track_added_to_chat(update: Update, context):
    for member in update.message.new_chat_members:
        if member.id == context.bot.id:
            chat_title = update.effective_chat.title
            welcome_group_text = (
                f"🎉 <b>Thanks for adding me to {chat_title}!</b>\n\n"
                f"🤖 I am <b>TYNEX VIDEO DOWNLOADER</b>.\n"
                f"👑 Developed by: <b>𝚪𝐘 𝚼𝚫𝚳𝚰𝚴</b>\n"
                f"🏆 Sponsored by: <b>TYNEX OFFICIAL</b>\n\n"
                f"💡 <i>To download videos without watermark, start a private chat with me!</i>"
            )
            keyboard = [[InlineKeyboardButton("📥 Open Bot Privately", url=f"https://t.me/{context.bot.username}?start=true")]]
            await update.message.reply_text(welcome_group_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

# ৫. ভিডিও ডাউনলোড প্রসেসিং
...
# ৬. পোস্ট-ইনিশিয়ালাইজেশন (শুধুমাত্র /start কমান্ড সেট করার জন্য)
async def post_init(application):
    commands = [
        ("start", "Start the TYNEX Downloader bot")
    ]
...
def main():
    ...
    # শুধুমাত্র /start স্ল্যাশ কমান্ড রাখা হলো
    application.add_handler(CommandHandler("start", start_command))
    application.add_handler(CallbackQueryHandler(button_callback))
    
    # গ্রুপ চ্যাট ট্র্যাকিং এবং টেক্সট মেসেজ লিংক প্রসেসিং
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, track_added_to_chat))
    application.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), process_and_download))

    print("🔥 TYNEX VIDEO DOWNLOADER Bot is running with clean /start command...")
